		...
		foreach($products as $k=>$p)
		{
			...
			if(isset($type_ingr) && strlen($type_ingr)>0)
			{
				foreach($p['items'] as $kk=>$vv)
				{
					...
					{
						$p['items'][$kk]['ingrids'][$ingrids[$type_ingr][$i][$kk]['id_menu']] = $ingrids[$type_ingr][$i][$kk];  // массив ингредиентов с ключами id_menu
					}
				}					
			}
			else
			...
			$p['sostav'] = $this->get_data('Get_ingrids', array($p['id_menu']));
			$sostav_text = '';
			$i = 0;
			$c = count($p['sostav']);
